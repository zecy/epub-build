...
for j in range(0, chapter_nums):
    ...
    if j < len(title_line_nums) - 1:
        # 取得每章正文结束的行号，实际多了一行。因为后续要使用list输出
        chap_con_end_num = title_line_nums[j + 1]
        chap_con_pre = html_doc[
            chap_con_start_num:chap_con_end_num]  # 通过行号把正文赋到一个变量。
    elif j == len(title_line_nums) - 1:
        chap_con_pre = html_doc[chap_con_start_num:]
    # 因为上面得到的是list，因此需要转为字符串，加四个空格做缩进，代码更整齐。
...
for l in range(0, len(fileslist)):
    ...
    if 'html' in thefile:
        manifest_out_pre.append('<item href=\"' + thefile + '\" id=\"' +
                                thefile + '\" media-type=\"application/xhtml+xml\" />\n')
        spine_out_pre.append('<itemref idref=\"' + thefile + '\" />\n')
        ncx_nav_list.append(thefile)

    elif 'css' in thefile:
        manifest_out_pre.append('<item href=\"style/' + thefile +
                                '\" id=\"' + thefile + '\" media-type=\"text/css\" />\n')

    elif 'jpg' in thefile:
        manifest_out_pre.append('<item href=\"images/' + thefile +
                                '\" id=\"x' + thefile + '\" media-type=\"image/jpeg\" />\n')

    elif 'png' in thefile:
        manifest_out_pre.append('<item href=\"images/' + thefile +
                                '\" id=\"x' + thefile + '\" media-type=\"image/png\" />\n')

    elif 'gif' in thefile:
        manifest_out_pre.append('<item href=\"images/' + thefile +
                                '\" id=\"x' + thefile + '\" media-type=\"image/gif\" />\n')
